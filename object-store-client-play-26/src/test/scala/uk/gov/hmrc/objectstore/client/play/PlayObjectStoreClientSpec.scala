...
class PlayObjectStoreClientSpec
  ...
  "zip" must {
    "return an ObjectListing with objectSummaries" in {
      ...
      initZipStub(from, to, retentionPeriod, defaultOwner, statusCode = 200, Some(zipResponse))
    ...
    "return an exception if object-store response is not successful" in {
      ...
      initZipStub(from, to, retentionPeriod, defaultOwner, statusCode = 401, response = None)
