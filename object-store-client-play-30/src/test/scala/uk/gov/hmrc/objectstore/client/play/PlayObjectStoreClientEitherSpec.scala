...
class PlayObjectStoreClientEitherSpec
  ...
  "presignedDownloadUrl" should {
    "return a PresignedDownloadUrl for an object that exists" in {
      ...
      osClient.presignedDownloadUrl(path).futureValue.value shouldBe response
    }
    ...
    "return an exception if the object doesn't exist" in {
      ...
      osClient.presignedDownloadUrl(path).futureValue.left.value shouldBe an[UpstreamErrorResponse]
    }
